...
import { retoureReasons } from 'src/configs/eventEnums';
...
import { RETURN_RECEIPT } from 'src/types/events/ReturnReceipt';

const props = defineProps<{ event: OPERATION_EVENT_DTO; details: RETURN_RECEIPT | null; currency: string }>();
...
const shortDetails = [
  ...
  {
    label: t('RETOURE_REASON'),
    value: retoureReasons.find((reason) => reason.value === props.details?.ReturnReason)?.label ?? '',
    couldBeCopied: false,
  },
];
  ...
  >
    ...
    <template #expanded-details>
      <EventPositions
        ...
        :rows="props.details?.ReturnReceiptPositions ?? []"
      />
    </template>
  </EventDetails>
</template>
