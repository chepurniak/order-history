...
import { DELIVERY_RECEIPT } from 'src/types/events/DeliveryReceipt';

const props = defineProps<{ event: OPERATION_EVENT_DTO; details: DELIVERY_RECEIPT | null; currency: string }>();
  ...
  >
    ...
    <template #expanded-details>
      <EventPositions
        ...
        :rows="props.details?.DeliveryReceiptPositions ?? []"
      />
    </template>
  </EventDetails>
</template>
